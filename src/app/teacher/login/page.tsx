...
export default function TeacherLoginPage() {
  ...
  const requestOtp = useAction(api.otp.requestTeacherOtp);
  ...
  useEffect(() => {
    if (!isLoading && isAuthenticated) router.replace("/teacher/dashboard");
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    if (!trimmedId) { toast.error("Please enter your Teacher ID."); return; }
    ...
    setLoading(true);
    try {
      const result = await requestOtp({ teacherId: trimmedId, emailOrPhone: trimmedInput });
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await signIn("teacher", { teacherId: teacherId.trim(), emailOrPhone: emailOrPhone.trim(), otp: trimmedOtp });
  ...
  const handleResendOtp = async () => {
    ...
    setLoading(true);
    try {
      const result = await requestOtp({ teacherId: teacherId.trim(), emailOrPhone: emailOrPhone.trim() });
      setMaskedEmail(result.maskedEmail);
      setResendCooldown(60);
      setOtp("");
          ...
          {step === "credentials" ? (
            <form onSubmit={handleRequestOtp} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="teacherId" value={teacherId} onChange={(e) => setTeacherId(e.target.value)} placeholder="TCH-0001" required autoComplete="username" />
              </div>
              <div className="space-y-2">
              ...
              <div className="text-center">
                <Link href="/teacher/forgot-password" className="text-sm text-primary hover:underline">Forgot Password?</Link>
              </div>
            </form>
          ) : (
